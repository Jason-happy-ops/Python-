def length1(list):          #贪心算法求最大连续子数列，这种方法只能计算连续的递增最大长度
    ...
    end_list = [list[0]]           #初始化列表用于保存元素
    for i in range(len(list)):
        max_length = 1          #记录以list[i]为起点的最长子序列长度
        max_list = [list[i]]           #记录以list[i]为起点的最长子序列   
        ...
        for j in range(i+1,len(list)):
            if last_num < list[j]:
                max_length += 1
                last_num = list[j]
                max_list.append(list[j])
        if max_length > global_length:
            global_length = max_length
            end_list = max_list
    print("最长连续子序列为:",end_list)
...
print(length2_global([5,1,2,4,3,6,7,1]))  # 全局: 
